...
async def predict(
    ...
    try:
        # File reading made secure
        contents = await file.read()
        df = pd.read_csv(io.BytesIO(contents))

        if len(df) < 60:
            return JSONResponse(content={"error": "Data is too short. Minimum 60 rows required in CSV."})

        workload = df.iloc[:,0].values
        
        if workload.max() == workload.min():
            return JSONResponse(content={"error": "All values in the dataset are the same. Cannot make predictions."})
        ...
        if len(df_temp) == 0:
            return JSONResponse(content={"error": "Not enough valid data after calculating moving averages. Check your CSV."})
        ...
        return JSONResponse(content={"error": "Invalid format"})

    except Exception as e:
        # YAHAN AAYEGA ASAL ERROR CATCH
        error_details = traceback.format_exc()
        print("Backend Error:", error_details) 
        return JSONResponse(content={"error": f"System Error: {str(e)}. Please check your CSV format."})